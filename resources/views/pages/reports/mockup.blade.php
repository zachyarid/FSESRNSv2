...
@section('content')
    <div class="row">
        <div class="col-xs-12 col-md-6 col-lg-6 col-xl-3">
            <div class="card-box tilebox-two">
                <button class="btn btn-sm btn-custom waves-effect waves-light pull-xs-right" data-toggle="modal" data-target="#byPilotModal">View</button>
                <h4 class="text-muted text-uppercase m-b-15">By Pilot</h4>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-xs-12 col-lg-12 col-xl-8">
            <div class="card-box">
                <h4 class="text-muted text-uppercase header-title m-t-0 m-b-20">Historic Group/Pilot Profit</h4>
    ...
    <div id="byPilotModal" class="modal fade" tabindex="-1" role="dialog" style="display: none;">
        <div class="modal-dialog modal-lg" style="width:90%;">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    <h4 class="modal-title text-lg-center" id="myModalLabel">By Pilot</h4>
                </div>
                <div class="modal-body">
                    <div class="table-responsive">
                        <table id="by_pilot" class="table table-striped table-bordered dataTable no-footer" style="width:100%">
                            <thead>
                            <tr>
                                <th>Pilot</th>
                                <th>Pilot Profit</th>
                            ...
                            <tbody>
                                @forelse ($reportData->flightsByPilot as $k => $v)
                                    <tr>
                                        <td>{{ $k }}</td>
                                        <td>${{ number_format($v['pilot_fee'],0) }}</td>
                                        <td>${{ number_format($v['income']-$v['pilot_fee']-$v['crew_cost']-$v['booking_fee']+$v['bonus']-$v['gcf']-$v['rental_cost'],0) }}</td>
                                        <td>${{ number_format($v['fuel_payments'],0) }}</td>
                                        <td>${{ number_format($v['income']-$v['pilot_fee']-$v['crew_cost']-$v['booking_fee']+$v['bonus']-$v['gcf']-$v['rental_cost']-$v['fuel_payments'],0) }}</td>
                                        <td>{{ number_format($v['count'],0) }}</td>
                                        <td>{{ number_format($v['distance'],0) }} nm</td>
                                        <td>${{ number_format(($v['income']-$v['pilot_fee']-$v['crew_cost']-$v['booking_fee']+$v['bonus']-$v['gcf']-$v['rental_cost']-$v['fuel_payments'])/$v['distance'],2) }}</td>
                                        <td>${{ number_format(($v['income']-$v['pilot_fee']-$v['crew_cost']-$v['booking_fee']+$v['bonus']-$v['gcf']-$v['rental_cost']-$v['fuel_payments'])/$v['count'],0) }}</td>
                                        <td>${{ number_format(($v['income']-$v['pilot_fee']-$v['crew_cost']-$v['booking_fee']+$v['bonus']-$v['gcf']-$v['rental_cost']-$v['fuel_payments'])/$v['flight_time'],0) }}</td>
    ...
    <div id="byAircraftModal" class="modal fade" tabindex="-1" role="dialog" style="display: none;">
        <div class="modal-dialog modal-lg" style="width:90%;">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="table-responsive">
                        <table id="by_aircraft" class="table table-striped table-bordered dataTable no-footer" style="width:100%">
                            <thead>
                            <tr>
                                <th>Aircraft</th>
                                <th>Type</th>
                ...
                <div class="modal-body">
                    <div class="table-responsive">
                        ...
                        <table id="leg_profit" class="table table-striped table-bordered" style="width:100%">
                            ...
                            <tbody>
                                @forelse ($reportData->flightsByProfitablilty as $k => $v)
                                    <tr>
                                        <td>{!! $k !!}</td>
                                        <td>${{ number_format($v['income'], 2) }}</td>
                                        <td>{{ number_format($v['count'], 0) }}</td>
                                    </tr>
                                @empty
